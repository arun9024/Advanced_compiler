...
class SemanticAnalyzer extends DepthFirstVisitor {
  SymbolTable _symTab;
  static final PrimitiveType BOOL_TYPE =
    new PrimitiveType(null, PrimitiveType.BOOL);
  static final PrimitiveType INT_TYPE =
    new PrimitiveType(null, PrimitiveType.INT);
  static final NullType NULL_TYPE = new NullType();
  static final PrimitiveType STRING_TYPE =
    new PrimitiveType(null, PrimitiveType.STRING);
  static final PrimitiveType VOID_TYPE =
    new PrimitiveType(null, PrimitiveType.VOID);

  SemanticAnalyzer(SymbolTable symTab) { _symTab = symTab; }

  static boolean knownType(Type type) {
    if (null == type)
      return false;
  ...
  static boolean sameType(Type t1, Type t2) {
    ...
    if (t1 instanceof RecordType) {
      ...
      if (r1._fields.size() != r2._fields.size())
        return false;
      for (int i=0, n=r1._fields.size(); i<n; i++)
        if (!sameType(r1._fields.get(i), r2._fields.get(i)))
          return false;
      return true;
    }
    if (t1 instanceof FieldType) {
      FieldType f1 = (FieldType)t1, f2 = (FieldType)t2;
      return f1._field._id.equals(f2._field._id) &&
        sameType(f1._type, f2._type);
    }
    if (t1 instanceof FunType) {
      FunType f1 = (FunType)t1, f2 = (FunType)t2;
      return sameType(f1._formals, f2._formals)
        && sameType(f1._returnType, f2._returnType);
    }
  ...
  Object visit(IfStmt ast) {
    ...
    if (!sameType(condType, BOOL_TYPE))
  ...
  Object visit(InfixExpr ast) {
    ...
    if ("||".equals(ast._op) || "&&".equals(ast._op)) {
      ast._type = BOOL_TYPE;
      if (!sameType(lhsType, BOOL_TYPE))
        ErrorPrinter.print(ast._lhs._loc, "Boolean expected");
      if (!sameType(rhsType, BOOL_TYPE))
      ...
      ast._type = BOOL_TYPE;
      if (!sameType(lhsType, INT_TYPE))
        ErrorPrinter.print(ast._lhs._loc, "Integer expected");
      if (!sameType(rhsType, INT_TYPE))
      ...
      if (sameType(lhsType, STRING_TYPE) || sameType(rhsType, STRING_TYPE)) {
        ...
        if (!sameType(rhsType, STRING_TYPE)) {
          if (castable(rhsType, STRING_TYPE))
            ast._rhs = new CastExpr(ast._rhs, STRING_TYPE);
          else
  ...
  Object visit(PrefixExpr ast) {
    ...
    if ("-".equals(ast._op)) {
      ast._type = INT_TYPE;
      if (!sameType(baseType, INT_TYPE))
        ErrorPrinter.print(ast._base._loc, "Integer expected");
    } else if ("!".equals(ast._op)) {
      ast._type = BOOL_TYPE;
      if (!sameType(baseType, BOOL_TYPE))
  ...
  Object visit(SubscriptExpr ast) {
    ...
    if (null != subscriptType) {
      if (!sameType(subscriptType, INT_TYPE))
